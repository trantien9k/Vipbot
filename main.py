...
#Lấy tại BotFather
API_BOT = '7441854605:AAG0yh9txglZJDzD3fWP2Dl8-JqTMBTpzNY'
...
# Danh sách các ID của admin được cấu hình trước
admin_ids = [6341479907, 5191030246]  # Thay các ID này bằng ID của admin thực tế
...
def show_main_menu(msg):
    ...
    photo_url = "https://i.imgur.com/59cGAl1.png"
    caption = """
<b>Chào Mừng Bạn Đã Đến Với Sân Chơi Giải Trí</b>
     <code>SAOWIN Game Bài Game Bài Uy Tín</code>
...
🎗 Tham gia nhóm: https://t.me/camaptds để nhận Giftcode hàng ngày nhé! 
...
🏧 Số Tài Khoản: <code>0337044339</code>

🏧Chủ Tài Khoản: <code>NGUYEN THUY TRANG</code>
...
🏧 Số Tài Khoản: <code>?</code>

🏧 Chủ Tài Khoản: <code>?</code>
...
/rutbank MBB 1234567890 NguyenVanA 10000
...
➡️ VD  /rutmomo 0123456789 200000
    ...
    try:
        ...
        if user_history:
            bot.send_message(
                ...
                f"Loại | UID | Số Tiền | Ngân Hàng | STK | TÊN |\n{user_history}"
            )
        else:
...
def set_balance(msg):
  if msg.from_user.id == 6341479907:
    bot.reply_to(msg, """
...
"""
    ...
    group_chat_id = -1002142338058  # Thay thế bằng ID thực sự của nhóm chat
...
def dice(message):
    ...
    # Now you can use the bet_amount variable to proceed with the game logic
    user_id = message.from_user.id
    if user_id not in user_balance:
        user_balance[user_id] = 0
...
""", parse_mode='HTML')
    ...
    current_state = "Dice Value"  # Cần cập nhật thông tin loại cược
    with open("lichsucuoc.txt", "a") as history_file:
        history_entry = {
            "user_id": user_id,
            "bet_type": current_state,
...
#----------------------------------------------------------------------------------------
bot2 = telebot.TeleBot('7325896707:AAE1kvPLQGK9X-7Vsioc_fiaJUXCuKMkUGI')


def send_dice_room(group_chat_id2):
    response = requests.get(
        f'https://api.telegram.org/bot7325896707:AAE1kvPLQGK9X-7Vsioc_fiaJUXCuKMkUGI/sendDice?chat_id=-1002207962317'
    )
    if response.status_code == 200:
        data = response.json()
        if 'result' in data and 'dice' in data['result']:
...
def notify_bet_success(user_id, bet_type, bet_amount):
    ...
    bot.send_message(-1002229615504, bet_message)

# Modify the confirm_bet function to call notify_bet_success
...
def send_game_result_and_process_winnings(result, dice_sum, game_result):
    ...
    # Sort users by their winnings in descending order
    ...
    bot2.send_message(-1002229615504, result_message, parse_mode='HTML')
    keyboard = types.InlineKeyboardMarkup()
    url_button = types.InlineKeyboardButton(text="Kết Quả TX [ Room ]",
                                            url="https://t.me/ketquazoom99")
...
def handle_message(message):
    ...
    # Check if bets are being accepted
    if accepting_bets:
        chat_id = message.chat.id
        ...
        # If bets are not being accepted, inform the user
